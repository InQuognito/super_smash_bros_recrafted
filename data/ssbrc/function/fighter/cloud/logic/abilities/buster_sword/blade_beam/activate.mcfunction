advancement revoke @s only ssbrc:utility/use_item/fighter/cloud/blade_beam

function ssbrc:logic/fighter/ability/init_entity {fighter:"cloud",item:"blade_beam"}

scoreboard players operation limit temp = @s resource
execute rotated ~ 0.0 positioned ^ ^ ^0.5 summon minecraft:marker run function ssbrc:fighter/cloud/logic/abilities/buster_sword/blade_beam/init
