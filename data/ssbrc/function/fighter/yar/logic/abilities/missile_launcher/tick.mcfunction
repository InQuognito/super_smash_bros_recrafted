...
execute unless block ^ ^ ^0.1 #ssbrc:passthrough run function ssbrc:fighter/yar/logic/abilities/missile_launcher/explode

execute positioned ~-0.25 ~-0.25 ~-0.25 as @n[tag=!self,predicate=ssbrc:flag/targets,dx=0] positioned ~-0.5 ~-0.5 ~-0.5 if entity @s[dx=0] at @n[type=minecraft:item_display,tag=missile_launcher,predicate=ssbrc:id_match] run function ssbrc:fighter/yar/logic/abilities/missile_launcher/explode
...
execute if entity @s[scores={temp=100..}] run function ssbrc:fighter/yar/logic/abilities/missile_launcher/explode
