...
execute unless data entity @s data{state:"stop"} unless block ^ ^ ^1 #ssbrc:passthrough run function ssbrc:fighter/yar/logic/abilities/drone/commands/stop/drone_force
...
execute if data entity @s[scores={cooldown=..0}] data{state:"stop"} facing entity @n[tag=!self,predicate=ssbrc:target,distance=0.1..3] eyes run function ssbrc:fighter/yar/logic/abilities/drone/projectile/activate

execute if data entity @s data{state:"recall"} positioned ~-0.5 ~-0.5 ~-0.5 as @a[tag=self,limit=1,dx=0] run function ssbrc:fighter/yar/logic/abilities/drone/recall
