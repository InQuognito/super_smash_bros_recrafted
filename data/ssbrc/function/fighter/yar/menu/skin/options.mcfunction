...
execute unless data storage ssbrc:temp player.temp_data{skin:"recruit"} run tellraw @s[advancements={ssbrc:fighter/yar/recruit=true}] {"translate":"ssbrc.skin.recruit","color":"dark_green","click_event":{"action":"run_command","command":"trigger menu set 3"}}
