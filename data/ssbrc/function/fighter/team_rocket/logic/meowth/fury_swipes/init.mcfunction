...
execute if score n temp matches 2.. as @e[tag=!self,predicate=ssbrc:flag/targets,dx=0] run function ssbrc:logic/damage/generic {amount:"0.75",type:"generic",kb_resist:"0.95"}
execute if score n temp matches 1 as @e[tag=!self,predicate=ssbrc:flag/targets,dx=0] run function ssbrc:logic/damage/generic {amount:"1.5",type:"generic",kb_resist:"0.0",source:" by @a[tag=self,limit=1"}
