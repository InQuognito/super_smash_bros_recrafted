...
execute unless data storage ssbrc:temp player.temp_data{skin:"hyrule_warriors"} run tellraw @s[advancements={ssbrc:fighter/zelda/hyrule_warriors=true}] {"translate":"ssbrc.skin.hyrule_warriors","color":"light_purple","clickEvent":{"action":"run_command","value":"/trigger menu set 3"}}
