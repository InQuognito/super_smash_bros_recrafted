...
execute positioned ~-0.15 ~ ~-0.15 as @e[tag=!self,predicate=ssbrc:flag/targets,dy=2] positioned ~-0.7 ~ ~-0.7 if entity @s[dy=2] run function ssbrc:logic/damage/fire {amount:"4.0",burning:"60",type:"pierce",kb_resist:"0.0",source:" by @a[tag=self,limit=1]"}
