...
execute if entity @s[tag=!silenced,tag=!robo_burner,scores={resource=18..,jump.cooldown=..0},predicate=ssbrc:input/jump] run function ssbrc:fighter/rob/logic/abilities/robo_burner/activate
execute if entity @s[tag=!silenced,tag=robo_burner,scores={resource=1..,jump.cooldown=..0},predicate=ssbrc:input/jump] run function ssbrc:fighter/rob/logic/abilities/robo_burner/tick
