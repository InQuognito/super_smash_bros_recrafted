...
particle minecraft:flame ^ ^ ^.25 0.05 0.05 0.05 0.0 2 normal @a
...
execute positioned ~-0.375 ~-0.375 ~-0.375 as @e[tag=!self,predicate=ssbrc:flag/targets,dx=0] positioned ~-0.25 ~-0.25 ~-0.25 if entity @s[dx=0] run function ssbrc:logic/damage/fire {amount:"4.0",burning:"30",type:"generic",kb_resist:"0.0",source:" by @a[tag=self,limit=1"}
...
execute if entity @s[scores={point=1..}] unless block ~ ~-0.6 ~ #ssbrc:passthrough run function ssbrc:fighter/mario/logic/abilities/fireball/bounce
