...
execute if items entity @s weapon.mainhand minecraft:nether_star[minecraft:custom_data~{item:"collarbone_breaker"},minecraft:damage~{damage:0}] if items entity @s weapon.offhand minecraft:nether_star[minecraft:custom_data~{item:"collarbone_breaker"},minecraft:damage~{damage:0}] if score hadouken temp < ryu.hadouken.limit const run function ssbrc:fighter/ryu/logic/abilities/hadouken/activate
