execute as @e[tag=!self,predicate=ssbrc:flag/targets,distance=..2] run function ssbrc:logic/damage/generic {amount:"4.0",type:"pierce",kb_resist:"0.0",source:" by @a[tag=self,limit=1]"}
