# Guardian Orbitars
execute if items entity @s[tag=!silenced,tag=!guardian_orbitars,scores={cooldown.2=..0},predicate=ssbrc:flag/sneaking] weapon.mainhand minecraft:bow[minecraft:custom_data~{item:"palutena_bow"}] positioned ~ ~0.75 ~ rotated ~ 0.0 run function ssbrc:fighter/pit/logic/abilities/guardian_orbitars/activate
...
execute if entity @s[tag=!silenced,scores={timer=1..}] run function ssbrc:fighter/pit/logic/abilities/wings/regain/timer
