...
execute unless data storage ssbrc:temp player.temp_data{skin:"bowsette"} run tellraw @s[advancements={ssbrc:fighter/bowser/bowsette=true}] {"translate":"ssbrc.skin.bowsette","color":"yellow","click_event":{"action":"run_command","command":"trigger menu set 3"}}
