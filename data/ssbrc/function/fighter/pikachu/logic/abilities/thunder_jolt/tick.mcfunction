...
execute positioned ~-0.3 ~-0.3 ~-0.3 as @e[tag=!self,predicate=ssbrc:flag/targets,dx=0] positioned ~-0.4 ~ ~-0.4 if entity @s[dx=0] run function ssbrc:logic/damage/player {amount:"6.0",type:"generic",kb_resist:"0.0"}
...
execute if entity @s[scores={point=1..}] unless block ~ ~-0.15 ~ #ssbrc:passthrough run function ssbrc:fighter/pikachu/logic/abilities/thunder_jolt/bounce
