...
execute unless items entity @s[tag=beat_call] weapon.mainhand minecraft:nether_star[minecraft:custom_data~{item:"beat_call"}] if entity @n[type=minecraft:bee,tag=beat,tag=!attack,tag=!forward,tag=!recall,predicate=ssbrc:id_match] run function ssbrc:fighters/mega_man/logic/abilities/beat_call/deactivate
