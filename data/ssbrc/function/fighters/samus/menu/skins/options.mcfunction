...
execute unless data storage ssbrc:temp player.temp_data{skin:"gravity_suit"} run tellraw @s[advancements={ssbrc:fighters/samus/skins/gravity_suit=true}] {"translate":"ssbrc.skin.gravity_suit","color":"light_purple","clickEvent":{"action":"run_command","value":"/trigger menu set 3"}}
execute if data storage ssbrc:temp player.temp_data{skin:"gravity_suit"} run tellraw @s [{"translate":"ssbrc.skin.gravity_suit","color":"light_purple"},{"text":" ✔","color":"green"}]
