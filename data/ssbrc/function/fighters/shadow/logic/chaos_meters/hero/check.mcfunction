...
execute unless items entity @s container.* minecraft:nether_star[minecraft:custom_data~{item:"chaos_control"}] if score @s shadow.meter.hero >= shadow.chaos_meter.max const run loot replace entity @s hotbar.4 loot ssbrc:fighters/shadow/chaos_control
