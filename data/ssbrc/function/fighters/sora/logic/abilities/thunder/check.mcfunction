...
execute unless data storage ssbrc:temp player.temp_data{form:"wisdom"} if score @s magic >= sora.spell.cost temp rotated ~ 0.0 run function ssbrc:fighters/sora/logic/abilities/thunder/thundaga/activate
execute if data storage ssbrc:temp player.temp_data{form:"wisdom"} if score @s magic >= sora.spell.cost temp anchored eyes positioned ^ ^ ^ run function ssbrc:fighters/sora/logic/abilities/thunder/thundaza/activate
