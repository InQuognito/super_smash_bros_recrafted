...
execute if entity @s[tag=stop,scores={cooldown.1=..0}] facing entity @n[tag=!self,predicate=ssbrc:flag/targets,distance=0.1..3] eyes run function ssbrc:fighters/yar/logic/abilities/drone/projectile/activate
...
execute if score in_electric_terrain temp matches 1 run function ssbrc:fighters/yar/logic/abilities/drone/kill
