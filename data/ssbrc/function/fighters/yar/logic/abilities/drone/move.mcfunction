execute if score percentage temp matches ..10 run teleport @s ^ ^ ^0.03
...
execute if score percentage temp matches 91.. run teleport @s ^ ^ ^0.30

scoreboard players remove @s[tag=!recall] temp 1
scoreboard players add @s[tag=recall,scores={temp=..60}] temp 1

execute if score @s temp matches 0 run function ssbrc:fighters/yar/logic/abilities/drone/commands/stop/drone_force
