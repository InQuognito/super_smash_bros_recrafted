...
execute if data storage ssbrc:temp player.temp_data{highlighted_object:"random_fighter"} run return run function ssbrc:fighters/random/get
...
$execute unless data storage ssbrc:temp player.temp_data{stage_vote:"$(highlighted_object)"} if entity @n[distance=..0.1,tag=stage_icon,name=!random_stage] run return run function ssbrc:logic/stages/vote with storage ssbrc:data stages.$(highlighted_object)
...
execute if data storage ssbrc:temp player.temp_data{highlighted_object:"PlanetMinecraft"} run return run tellraw @s [{"text":"[","color":"dark_green","bold":true},{"text":"PlanetMinecraft","color":"green","clickEvent":{"action":"open_url","value":"https://www.planetminecraft.com/member/team_recrafted"}},{"text":"]"}]
...
$execute if entity @n[distance=..0.1,tag=developer] run return run function ssbrc:logic/selector/events/developers/$(highlighted_object)
