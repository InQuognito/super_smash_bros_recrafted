...
$execute as @a[tag=!leaderboard.loaded,scores={stats.$(value)=1..}] if score @s stats.$(value) = leaderboard.value.$(n) temp if score @s stats.$(ratio) = leaderboard.ratio.$(n) temp run function ssbrc:logic/lobby/leaderboard/set with storage ssbrc:leaderboard
...
$execute if score counter temp matches ..5 if entity @a[tag=!leaderboard.loaded,scores={stats.$(value)=1..}] run function ssbrc:logic/lobby/leaderboard/check with storage ssbrc:leaderboard
