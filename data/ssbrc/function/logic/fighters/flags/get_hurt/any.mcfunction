...
execute if data storage ssbrc:temp player.temp_data{fighter:"jigglypuff"} if entity @s[tag=rest] if predicate ssbrc:random_chance/15 run function ssbrc:fighters/jigglypuff/logic/abilities/rest/deactivate
...
execute if data storage ssbrc:temp player.temp_data{fighter:"team_rocket"} run loot replace entity @s[tag=wobbuffet] hotbar.0 loot ssbrc:fighters/team_rocket/wobbuffet/counter
execute if data storage ssbrc:temp player.temp_data{fighter:"yar"} if items entity @s[scores={charge.output=1..}] weapon.mainhand minecraft:nether_star[minecraft:custom_data~{item:"gatling_gun",power_up:"false"}] run function ssbrc:logic/resets/charge/abilities
