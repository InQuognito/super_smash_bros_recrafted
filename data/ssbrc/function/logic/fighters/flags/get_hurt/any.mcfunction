...
execute if data storage ssbrc:temp player.temp_data{fighter:"link"} if items entity @s[scores={health=..39}] container.* minecraft:nether_star[minecraft:custom_data~{item:"master_sword",awakened:"true"}] run function ssbrc:logic/item/modify {search_key:"item",search_value:"master_sword",path:"ssbrc:fighters/link/master_sword/disable"}
