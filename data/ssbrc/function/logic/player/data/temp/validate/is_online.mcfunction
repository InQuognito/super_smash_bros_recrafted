$execute if entity @a[name=$(name),tag=!room.fighter_select,limit=1] run return run function ssbrc:logic/player/data/temp/validate/remove with storage ssbrc:data player.temp_data_copy[-1]
