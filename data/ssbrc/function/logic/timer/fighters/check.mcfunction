...
execute if data storage ssbrc:temp player.temp_data{fighter:"sora"} if score @s[tag=!valor,tag=!anti] magic < sora.magic.max const run function ssbrc:logic/timer/fighters/sora
execute if data storage ssbrc:temp player.temp_data{fighter:"zelda"} if score @s magic < zelda.magic.max const run function ssbrc:logic/timer/fighters/zelda
