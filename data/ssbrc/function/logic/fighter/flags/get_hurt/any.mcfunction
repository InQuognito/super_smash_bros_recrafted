...
execute if data storage ssbrc:temp player.temp_data{fighter:"king_k_rool"} run scoreboard players operation @s duration.1 = king_k_rool.adaptive_armor const
...
execute if data storage ssbrc:temp player.temp_data{fighter:"zelda"} if items entity @s[scores={health=..6}] container.* minecraft:nether_star[minecraft:custom_data~{item:"nayrus_ring"}] run function ssbrc:fighter/zelda/logic/get_hurt
