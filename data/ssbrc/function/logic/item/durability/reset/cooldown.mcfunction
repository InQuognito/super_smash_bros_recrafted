...
$execute if items entity @s weapon.offhand #ssbrc:equipment[minecraft:custom_data~{$(key):"$(value)"}] run data modify storage ssbrc:temp cache.item.cooldown set from entity @s Inventory[{Slot:$(slot_id)b}].components."minecraft:custom_data".$(source)
