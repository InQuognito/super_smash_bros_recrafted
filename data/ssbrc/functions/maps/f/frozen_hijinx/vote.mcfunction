...
execute unless entity @s[tag=frozenHijinx] run tellraw @a[tag=room.mapVoting] [{"selector":"@s","color":"yellow"},{"translate":"ssbrc.stageSelect.voteMap","color":"gold"},{"text":"Frozen Hijinx","color":"gold"},{"translate":"ssbrc.stageSelect.voteMap.display","color":"gold"},{"score":{"name":"@e[type=minecraft:text_display,tag=voteCounter,tag=frozenHijinx,limit=1]","objective":"mapVote"},"color":"yellow"}]
