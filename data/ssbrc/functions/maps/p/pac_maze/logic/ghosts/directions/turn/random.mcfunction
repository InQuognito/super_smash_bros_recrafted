...
execute if score result random matches 2 unless entity @s[tag=turned] unless block ^ ^7.0 ^1.0 minecraft:red_concrete at @s align xyz positioned ~.5 ~ ~.5 run function ssbrc:maps/p/pac_maze/logic/ghosts/directions/forward

execute unless entity @s[tag=turned] run function ssbrc:maps/p/pac_maze/logic/ghosts/directions/turn/random
tag @s remove turned
