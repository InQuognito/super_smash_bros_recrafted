...
execute as @e[tag=pacMaze.dot] at @s if entity @p[tag=alive,scores={respawn=..0},distance=..0.75] run function ssbrc:maps/p/pac_maze/logic/dots/pickup
execute as @e[tag=pacMaze.pellet] at @s if entity @p[tag=alive,scores={respawn=..0},distance=..1.0] run function ssbrc:maps/p/pac_maze/logic/pellets/pickup
