...
execute if entity @s[tag=firing] unless score @s cooldown.1 matches 1.. positioned ~-1.5 ~-3 ~-1.5 if entity @e[tag=!self,predicate=ssbrc:flag/targets,sort=nearest,limit=1,dx=2,dy=2,dz=2] positioned ~ ~2.5 ~ facing entity @e[tag=!self,predicate=ssbrc:flag/targets,sort=nearest,limit=1] eyes run function ssbrc:maps/p/pyrosphere/logic/fgii_graham/fire
