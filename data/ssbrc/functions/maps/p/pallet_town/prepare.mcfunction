...
execute as @e[type=minecraft:armor_stand,tag=diglett] if predicate ssbrc:shiny_chance run item replace entity @s armor.head with minecraft:diamond{CustomModelData:502}
execute as @e[type=minecraft:armor_stand,tag=dugtrio] if predicate ssbrc:shiny_chance run item replace entity @s armor.head with minecraft:diamond{CustomModelData:512}
