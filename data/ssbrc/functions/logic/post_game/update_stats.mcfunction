scoreboard players add @a[team=!spectator,tag=alteredbeast] alteredbeast.gP 1
scoreboard players add @a[team=!spectator,tag=bowser] bowser.gP 1
scoreboard players add @a[team=!spectator,tag=captainfalcon] captainfalcon.gP 1
scoreboard players add @a[team=!spectator,tag=charizard] charizard.gP 1
scoreboard players add @a[team=!spectator,tag=donkeykong] donkeykong.gP 1
scoreboard players add @a[team=!spectator,tag=fox] fox.gP 1
scoreboard players add @a[team=!spectator,tag=ganon] ganon.gP 1
scoreboard players add @a[team=!spectator,tag=greninja] greninja.gP 1
scoreboard players add @a[team=!spectator,tag=hero] hero.gP 1
scoreboard players add @a[team=!spectator,tag=joker] joker.gP 1
scoreboard players add @a[team=!spectator,tag=kirby] kirby.gP 1
scoreboard players add @a[team=!spectator,tag=kingkrool] kingkrool.gP 1
scoreboard players add @a[team=!spectator,tag=link] link.gP 1
scoreboard players add @a[team=!spectator,tag=mario] mario.gP 1
scoreboard players add @a[team=!spectator,tag=megaman] megaman.gP 1
scoreboard players add @a[team=!spectator,tag=ness] ness.gP 1
scoreboard players add @a[team=!spectator,tag=pit] pit.gP 1
scoreboard players add @a[team=!spectator,tag=ryu] ryu.gP 1
scoreboard players add @a[team=!spectator,tag=samus] samus.gP 1
scoreboard players add @a[team=!spectator,tag=shovelknight] shovelknight.gP 1
scoreboard players add @a[team=!spectator,tag=snake] snake.gP 1
scoreboard players add @a[team=!spectator,tag=sonic] sonic.gP 1
scoreboard players add @a[team=!spectator,tag=steve] steve.gP 1

tag @s add winner
scoreboard players reset @a[team=!spectator,tag=!winner,tag=alteredbeast] alteredbeast.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=bowser] bowser.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=captainfalcon] captainfalcon.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=charizard] charizard.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=donkeykong] donkeykong.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=fox] fox.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=ganon] ganon.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=greninja] greninja.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=hero] hero.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=joker] joker.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=kirby] kirby.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=kingkrool] kingkrool.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=link] link.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=mario] mario.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=megaman] megaman.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=ness] ness.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=pit] pit.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=ryu] ryu.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=samus] samus.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=shovelknight] shovelknight.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=snake] snake.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=sonic] sonic.wS
scoreboard players reset @a[team=!spectator,tag=!winner,tag=steve] steve.wS
tag @a remove winner

scoreboard players reset @s[tag=!alteredbeast] alteredbeast.wS
scoreboard players reset @s[tag=!bowser] bowser.wS
scoreboard players reset @s[tag=!captainfalcon] captainfalcon.wS
scoreboard players reset @s[tag=!charizard] charizard.wS
scoreboard players reset @s[tag=!donkeykong] donkeykong.wS
scoreboard players reset @s[tag=!fox] fox.wS
scoreboard players reset @s[tag=!ganon] ganon.wS
scoreboard players reset @s[tag=!greninja] greninja.wS
scoreboard players reset @s[tag=!hero] hero.wS
scoreboard players reset @s[tag=!joker] joker.wS
scoreboard players reset @s[tag=!kirby] kirby.wS
scoreboard players reset @s[tag=!kingkrool] kingkrool.wS
scoreboard players reset @s[tag=!link] link.wS
scoreboard players reset @s[tag=!mario] mario.wS
scoreboard players reset @s[tag=!megaman] megaman.wS
scoreboard players reset @s[tag=!ness] ness.wS
scoreboard players reset @s[tag=!pit] pit.wS
scoreboard players reset @s[tag=!ryu] ryu.wS
scoreboard players reset @s[tag=!samus] samus.wS
scoreboard players reset @s[tag=!shovelknight] shovelknight.wS
scoreboard players reset @s[tag=!snake] snake.wS
scoreboard players reset @s[tag=!sonic] sonic.wS
scoreboard players reset @s[tag=!steve] steve.wS

scoreboard players add @s[tag=alteredbeast] alteredbeast.wS 1
scoreboard players add @s[tag=bowser] bowser.wS 1
scoreboard players add @s[tag=captainfalcon] captainfalcon.wS 1
scoreboard players add @s[tag=charizard] charizard.wS 1
scoreboard players add @s[tag=donkeykong] donkeykong.wS 1
scoreboard players add @s[tag=fox] fox.wS 1
scoreboard players add @s[tag=ganon] ganon.wS 1
scoreboard players add @s[tag=greninja] greninja.wS 1
scoreboard players add @s[tag=hero] hero.wS 1
scoreboard players add @s[tag=joker] joker.wS 1
scoreboard players add @s[tag=kirby] kirby.wS 1
scoreboard players add @s[tag=kingkrool] kingkrool.wS 1
scoreboard players add @s[tag=link] link.wS 1
scoreboard players add @s[tag=mario] mario.wS 1
scoreboard players add @s[tag=megaman] megaman.wS 1
scoreboard players add @s[tag=ness] ness.wS 1
scoreboard players add @s[tag=pit] pit.wS 1
scoreboard players add @s[tag=ryu] ryu.wS 1
scoreboard players add @s[tag=samus] samus.wS 1
scoreboard players add @s[tag=shovelknight] shovelknight.wS 1
scoreboard players add @s[tag=snake] snake.wS 1
scoreboard players add @s[tag=sonic] sonic.wS 1
scoreboard players add @s[tag=steve] steve.wS 1

function ssbrc:logic/stats/advancements
