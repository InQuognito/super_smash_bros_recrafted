...
execute if score game_mode options matches 1 run tellraw @s [{"translate":"ssbrc.game.end.summary.duration","color":"gold"},{"score":{"name":"gameTime","objective":"timer"},"color":"yellow"},{"translate":"s","color":"gold"}]

execute if entity @s[tag=!admin,tag=!spectator] run function ssbrc:logic/post_game/summary/player
