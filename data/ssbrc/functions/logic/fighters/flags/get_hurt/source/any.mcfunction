...
execute if entity @s[tag=jigglypuff,tag=rest] if predicate ssbrc:random_chance/15 run function ssbrc:fighters/jigglypuff/logic/abilities/rest/deactivate
execute if entity @s[tag=link,nbt={Inventory:[{tag:{master_sword:1,awakened:1}}]},scores={health=..39}] run function ssbrc:fighters/link/logic/master_sword
...
execute if entity @s[tag=yar,nbt={SelectedItem:{tag:{gatling_gun:1}}},scores={charge.output=1..}] run function ssbrc:logic/resets/charge
execute if entity @s[tag=zelda,nbt={Inventory:[{tag:{nayrus_ring:1}}]},scores={health=..6}] run function ssbrc:fighters/zelda/logic/get_hurt
