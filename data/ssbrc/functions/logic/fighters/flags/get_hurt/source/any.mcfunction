...
execute if score gameStage temp matches 4 if entity @s[tag=team_rocket,tag=wobbuffet] run loot replace entity @s hotbar.0 loot ssbrc:fighters/team_rocket/wobbuffet/counter
...
execute if score gameStage temp matches 4 if entity @s[tag=zelda,nbt={Inventory:[{tag:{nayrusRing:1}}]},scores={health=..6}] run function ssbrc:fighters/zelda/logic/get_hurt
