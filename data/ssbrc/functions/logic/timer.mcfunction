...
execute if score $gameStage temp matches 1 if score $countdown timer matches -1 run function ssbrc:logic/pre_game/map_voting/goto

# Map Voting Countdown
execute if score $gameStage temp matches 2 unless score #votesLocked temp matches 1 run title @a actionbar [{"text":"Vote time remaining: ","color":"gold"},{"score":{"name":"$countdown","objective":"timer"},"color":"yellow"},{"text":"s","color":"gold"}]
...
# Start Countdown
execute if score $countdown timer matches 3 as @a at @s run playsound ssbrc:smash_ultimate_countdown voice @s
execute if score $countdown timer matches 3 run title @a title {"text":"3","color":"dark_red"}
execute if score $countdown timer matches 2 run title @a title {"text":"2","color":"red"}
execute if score $countdown timer matches 1 run title @a title {"text":"1","color":"gold"}
