...
execute if score characterPicked temp = players temp if score characterPicked temp = largest_team temp unless score players temp matches ..1 run function ssbrc:logic/pre_game/character_select/start/same_team

# All players have chosen their fighter, and pass the teams condition (Excluding singleplayer)
execute if score characterPicked temp = players temp unless score characterPicked temp = largest_team temp unless score players temp matches ..1 run function ssbrc:logic/pre_game/force_start
