...
scoreboard players set @e[type=minecraft:text_display,tag=voteCounter] stage_vote 0

execute as @e[type=minecraft:text_display,tag=voteCounter,tag=selected] unless score @s stage_vote = @s temp run tag @s add voteCounter.update
