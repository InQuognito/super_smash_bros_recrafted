...
execute if score fighter_picked temp = players temp if score fighter_picked temp = largest teamAmount unless score players temp matches ..1 run function ssbrc:logic/pre_game/fighter_select/start/same_team

# All players have chosen their fighter, and pass the teams condition (Excluding singleplayer)
execute if score fighter_picked temp = players temp unless score fighter_picked temp = largest teamAmount unless score players temp matches ..1 run function ssbrc:logic/pre_game/force_start
