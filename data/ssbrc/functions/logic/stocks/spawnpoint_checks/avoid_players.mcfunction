...
execute if entity @e[type=minecraft:marker,tag=checkSpawnpoint,sort=random,limit=1] if entity @a[tag=respawnMe,limit=1] as @e[type=minecraft:marker,tag=checkSpawnpoint,sort=random,limit=1] at @s run function ssbrc:logic/stocks/spawnpoint_checks/avoid_players
