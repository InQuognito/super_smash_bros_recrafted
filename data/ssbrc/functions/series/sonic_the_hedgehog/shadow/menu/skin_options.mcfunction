...
tellraw @s[advancements={ssbrc:series/sonic_the_hedgehog/shadow/skins/alt=true}] [{"text":"[ ","color":"dark_green"},{"text":"Alt","color":"yellow","clickEvent":{"action":"run_command","value":"/execute if entity @s[tag=room.characterSelect,tag=!alt] run function ssbrc:series/sonic_the_hedgehog/shadow/menu/skins/alt"}},{"text":" ]","color":"dark_green"}]
