...
tellraw @s[advancements={ssbrc:series/sonic_the_hedgehog/shadow/skins/yellow_android=true}] [{"text":"[ ","color":"dark_green"},{"text":"Yellow Android","color":"yellow","clickEvent":{"action":"run_command","value":"/execute if entity @s[tag=room.characterSelect,tag=!alt] run function ssbrc:series/sonic_the_hedgehog/shadow/menu/skins/yellow_android"}},{"text":" ]","color":"dark_green"}]
