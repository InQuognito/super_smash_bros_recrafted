...
tag @s add self
execute as @e[type=minecraft:arrow,tag=flareWand] at @s if score @s id = @p[tag=flareWand,tag=self] id run function ssbrc:series/indie_fighters/shovelknight/logic/magic/flare_wand/tick
execute as @e[type=minecraft:marker,tag=propellerDagger] at @s if score @s id = @p[tag=propellerDagger,tag=self,scores={duration.1=2..}] id run function ssbrc:series/indie_fighters/shovelknight/logic/magic/propeller_dagger/tick
tag @s remove self
