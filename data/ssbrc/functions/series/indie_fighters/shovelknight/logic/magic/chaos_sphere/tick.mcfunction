...
execute if score @s point < #shovelknight.chaosSphereMaxRotation vars run scoreboard players remove @s slope 5

execute if score @s point > 0 integers unless block ~ ~-0.15 ~ #ssbrc:passthrough run function ssbrc:series/indie_fighters/shovelknight/logic/magic/chaos_sphere/bounce

execute rotated as @s run teleport @s ^ ^ ^0.15
