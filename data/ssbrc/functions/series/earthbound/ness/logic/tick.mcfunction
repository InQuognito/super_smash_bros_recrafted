...
execute if entity @s[scores={useAbility=1..,cooldown.2=..0},nbt={SelectedItem:{tag:{pkFlash:1}}}] at @s anchored eyes run function ssbrc:series/earthbound/ness/logic/abilities/pk_flash/activate
...
execute unless entity @s[predicate=ssbrc:flag/sneaking] as @e[type=minecraft:area_effect_cloud,tag=pkFlash,scores={temp=40..}] if score @s id = @p[tag=self] id at @s run function ssbrc:series/earthbound/ness/logic/abilities/pk_flash/adjust
