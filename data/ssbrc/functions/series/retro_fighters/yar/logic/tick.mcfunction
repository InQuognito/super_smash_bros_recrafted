...
execute if entity @s[scores={charge.output=1..,cooldown.1=..0},nbt={SelectedItem:{tag:{gatlingGun:1}}}] at @s anchored eyes positioned ^ ^ ^ run function ssbrc:series/retro_fighters/yar/logic/abilities/ray_blaster/variants/gatling_gun/activate
execute if entity @s[scores={charge.step=5..,charge.output=40..,cooldown.1=..0},nbt={SelectedItem:{tag:{railgun:1}}}] at @s anchored eyes run function ssbrc:series/retro_fighters/yar/logic/abilities/ray_blaster/variants/railgun/raycast/start
...
# Gatling Gun
execute if entity @s[tag=!gold,nbt={SelectedItem:{tag:{gatlingGun:1}}}] unless score @s charge.output matches 1.. run loot replace entity @s weapon.mainhand loot ssbrc:characters/retro_fighters/yar/ray_blaster/gatling_gun/default/default
execute if entity @s[tag=gold,nbt={SelectedItem:{tag:{gatlingGun:1}}}] unless score @s charge.output matches 1.. run loot replace entity @s weapon.mainhand loot ssbrc:characters/retro_fighters/yar/ray_blaster/gatling_gun/gold/default
