...
execute positioned ~-0.3 ~-0.3 ~-0.3 as @e[predicate=ssbrc:flag/targets,dx=0] positioned ~-0.4 ~-0.4 ~-0.4 if entity @s[dx=0] unless score @s id = @e[type=minecraft:marker,tag=thunderJolt,sort=nearest,limit=1] id run function ssbrc:series/pokemon/pikachu/logic/abilities/thunder_jolt/hit
