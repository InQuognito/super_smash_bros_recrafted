...
execute if entity @s[scores={useAbility=1..},nbt={SelectedItem:{tag:{rest:1}}}] at @s run function ssbrc:series/pokemon/jigglypuff/logic/abilities/rest/activate

tag @s add self
execute as @e[type=minecraft:arrow,tag=waterShuriken] at @s if score @s id = @p[tag=self] id run function ssbrc:series/pokemon/jigglypuff/logic/abilities/hyper_voice/tick
tag @s remove self
