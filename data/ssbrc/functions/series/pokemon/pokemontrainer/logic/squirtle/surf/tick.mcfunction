...
execute at @s unless block ^ ^ ^0.1 #ssbrc:passthrough_charge_water run function ssbrc:series/pokemon/pokemontrainer/logic/squirtle/surf/deactivate
...
execute at @s if block ~ ~-0.1 ~ minecraft:air run teleport @s ~ ~-0.1 ~
execute at @s if block ~ ~-0.1 ~ minecraft:lava run teleport @s ~ ~-0.1 ~
