...
execute unless block ^ ^ ^0.1 #ssbrc:passthrough_charge_water run function ssbrc:series/pokemon/pokemontrainer/logic/squirtle/surf/deactivate
execute if block ^ ^ ^0.7 #ssbrc:passthrough_charge_water at @s run teleport @s ^ ^ ^0.6
execute if block ^ ^ ^0.4 #ssbrc:passthrough_charge_water at @s if block ~ ~-0.1 ~ minecraft:water run teleport @s ^ ^ ^0.3
