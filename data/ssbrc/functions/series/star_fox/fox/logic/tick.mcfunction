...
# Reflector
execute if score @s duration.2 matches 2.. at @s anchored eyes positioned ^ ^ ^2 as @e[type=#ssbrc:projectiles,predicate=!ssbrc:entity_kill_exceptions,tag=!reflected,tag=!barrel,tag=!link.boomerang,tag=!zelda.boomerang,tag=!magicBoomerang,distance=..2] run function ssbrc:series/star_fox/fox/logic/abilities/reflector/tick
