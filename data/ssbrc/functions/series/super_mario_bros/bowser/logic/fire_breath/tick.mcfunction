...
scoreboard players add @a[tag=fireBreath] bowser.fireBreath.hurt 1
...
execute as @a[scores={bowser.fireBreath.hurt=2..}] at @s run teleport @e[type=minecraft:area_effect_cloud,tag=bowser.fireBreath,sort=nearest,limit=1] ~ ~ ~
...
tag @a[scores={bowser.fireBreath.hurt=10..}] remove fireBreath
