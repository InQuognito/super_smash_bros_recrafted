...
tellraw @s[advancements={ssbrc:series/super_mario_bros/luigi/skins/flower_power=true}] [{"text":"[ ","color":"dark_green"},{"text":"Flower Power","color":"green","clickEvent":{"action":"run_command","value":"/execute if entity @s[tag=room.characterSelect,tag=!flower_power] run function ssbrc:series/super_mario_bros/luigi/menu/skins/flower_power"}},{"text":" ]","color":"dark_green"}]
