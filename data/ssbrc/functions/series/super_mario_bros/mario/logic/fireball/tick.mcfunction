...
execute unless score #pointSlope temp matches 70.. run scoreboard players operation @s point -= @s slope
execute unless score #pointSlope temp matches 70.. run scoreboard players remove @s slope 10

execute unless block ~ ~-0.5 ~ #ssbrc:passthrough run function ssbrc:series/super_mario_bros/mario/logic/fireball/bounce

execute rotated as @s run tp @s ^ ^ ^0.4
