...
execute positioned ~-0.45 ~-0.45 ~-0.45 as @a[tag=alive,scores={respawn=..0},gamemode=adventure,dx=0] positioned ~-0.1 ~-0.1 ~-0.1 if entity @s[dx=0] unless score @s id = @e[type=minecraft:marker,tag=fireball,sort=nearest,limit=1] id run tag @s add damage.fireball
