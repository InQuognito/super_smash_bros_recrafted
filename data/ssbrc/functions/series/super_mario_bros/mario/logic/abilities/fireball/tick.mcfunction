...
execute positioned ~-0.3 ~-0.3 ~-0.3 as @a[tag=alive,scores={respawn=..0},dx=0] positioned ~-0.4 ~-0.4 ~-0.4 if entity @s[dx=0] unless score @s id = @e[type=minecraft:marker,tag=fireball,sort=nearest,limit=1] id run tag @s add damage.fireball
