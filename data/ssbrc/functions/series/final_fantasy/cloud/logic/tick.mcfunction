...
execute if entity @s[tag=operator,predicate=!ssbrc:flag/sneaking,scores={useAbility=1..,cooldown.2=..0},nbt={SelectedItem:{tag:{busterSword:1}}}] at @s rotated ~ 0.0 run function ssbrc:series/final_fantasy/cloud/logic/blade_beam/activate
...
# Cross Slash
execute if score @s duration.1 matches 1.. at @s anchored eyes rotated ~ 0.0 positioned ^ ^ ^1 run function ssbrc:series/final_fantasy/cloud/logic/cross_slash/tick
