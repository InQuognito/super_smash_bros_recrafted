execute if score @s cloud.limit < #cloud.maxLimit vars positioned 0.0 0.0 0.0 run summon minecraft:marker ^ ^ ^1 {Tags:["direction"]}
execute if score @s cloud.limit >= #cloud.maxLimit vars positioned 0.0 0.0 0.0 run summon minecraft:marker ^ ^ ^1.5 {Tags:["direction"]}

execute if score @s cloud.limit < #cloud.maxLimit vars run summon minecraft:arrow ^ ^ ^ {damage:6.0,Tags:["bladeBeam","modifyProjectile"],NoGravity:1b}
execute if score @s cloud.limit >= #cloud.maxLimit vars run summon minecraft:arrow ^ ^ ^ {damage:10.0,Tags:["bladeBeam","limitBreak","modifyProjectile"],NoGravity:1b}

scoreboard players operation @e[tag=modifyProjectile,limit=1] id = @s id
data modify entity @e[tag=modifyProjectile,limit=1] Owner set from entity @s UUID
data modify entity @e[tag=modifyProjectile,limit=1] Motion set from entity @e[tag=direction,limit=1] Pos

tag @e[tag=modifyProjectile,limit=1] remove modifyProjectile
kill @e[tag=direction]
