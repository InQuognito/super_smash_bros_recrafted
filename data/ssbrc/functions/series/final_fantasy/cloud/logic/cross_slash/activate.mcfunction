...
tag @s add self
execute positioned ^ ^ ^1 as @p[tag=alive,scores={respawn=..0},gamemode=adventure,tag=!self,distance=..1.5] at @e[type=minecraft:marker,tag=crossSlash,sort=nearest,limit=1] run function ssbrc:series/final_fantasy/cloud/logic/cross_slash/target
tag @s remove self
