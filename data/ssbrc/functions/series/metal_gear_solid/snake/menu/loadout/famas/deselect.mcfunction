title @s[tag=selectingLoadout.snake,tag=!famas] actionbar {"text":"You do not have this equipped.","color":"red"}

scoreboard players operation @s[tag=selectingLoadout.snake,tag=famas] snake.capacity -= #snake.famasWeight vars
title @s[tag=selectingLoadout.snake,tag=famas] actionbar [{"text":"You have de-selected the Famas.","color":"red"}]
