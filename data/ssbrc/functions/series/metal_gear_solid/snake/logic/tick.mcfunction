...
execute as @e[type=minecraft:marker,tag=smokeGrenade,tag=active,sort=nearest,limit=1] at @s if entity @s[tag=nightVisionGoggles,distance=4..] run function ssbrc:series/metal_gear_solid/snake/logic/night_vision_goggles/deactivate
