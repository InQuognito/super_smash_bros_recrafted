...
execute positioned ~-0.25 ~-0.25 ~-0.25 as @e[tag=!self,nbt=!{Inventory:[{tag:{goronLocket:1}}]},predicate=ssbrc:flag/targets,dx=0] positioned ~-0.5 ~-0.5 ~-0.5 if entity @s[dx=0] run function ssbrc:series/the_legend_of_zelda/zelda/logic/abilities/fire_rod/hit
