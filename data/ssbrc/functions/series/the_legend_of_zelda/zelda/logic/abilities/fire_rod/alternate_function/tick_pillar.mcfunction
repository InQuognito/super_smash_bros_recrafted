...
execute positioned ~-0.15 ~ ~-0.15 as @e[predicate=ssbrc:flag/targets,dy=2] positioned ~-0.7 ~ ~-0.7 if entity @s[dy=2] unless score @s id = @e[type=minecraft:area_effect_cloud,tag=fireRod,sort=nearest,limit=1] id at @s run tag @s add damage.fireRod
