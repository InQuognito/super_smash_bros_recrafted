...
execute unless entity @s[tag=exploded] if score #percentage temp matches 100.. run function ssbrc:series/the_legend_of_zelda/zelda/logic/abilities/bomb/explode
execute if entity @s[tag=!exploded,tag=blasting] unless block ~ ~-0.1 ~ #ssbrc:passthrough run function ssbrc:series/the_legend_of_zelda/zelda/logic/abilities/bomb/explode
