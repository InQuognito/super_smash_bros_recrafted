...
# Rebel's Guard
execute at @s[tag=!rebelsGuard,predicate=ssbrc:flag/sneaking] if block ~ ~1.5 ~ #ssbrc:passthrough_charge run function ssbrc:series/persona/joker/logic/abilities/rebels_guard/activate
execute at @s[tag=rebelsGuard,predicate=ssbrc:flag/sneaking] unless block ~ ~1.5 ~ #ssbrc:passthrough_charge run function ssbrc:series/persona/joker/logic/abilities/rebels_guard/deactivate
