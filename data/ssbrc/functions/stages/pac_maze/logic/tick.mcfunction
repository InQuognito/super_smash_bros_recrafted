execute if score pac_mazeType stage matches 1 run function ssbrc:stages/pac_maze/logic/pac_maze/tick
execute if score pac_mazeType stage matches 2 run function ssbrc:stages/pac_maze/logic/mrs_pac_maze/tick
...
execute as @e[type=minecraft:item_display,tag=powerPellet] at @s if entity @p[predicate=ssbrc:flag/player,distance=..1.0] run function ssbrc:stages/pac_maze/logic/power_pellet/pickup

# Galaxian Flagship
scoreboard players add @a[tag=galaxian_flagship] timer.galaxian_flagship 1
execute as @a[scores={timer.galaxian_flagship=1..}] at @s run function ssbrc:stages/pac_maze/logic/fruit/galaxian_flagship/tick
