...
# Destroy Tower
execute if score tower_of_fate.destroyed temp matches 1.. if score tower_of_fate.destroyedTimer temp matches 1.. run function ssbrc:stages/tower_of_fate/logic/destroy_tower/tick
...
execute if score tower_of_fate.destroyed temp matches 2 as @a[predicate=ssbrc:flag/targets,predicate=ssbrc:above_y/22] at @s run function ssbrc:stages/tower_of_fate/logic/mystic_barrier/tick
