...
# Destroy Tower
execute if score tower_of_fateDestroyed temp matches 1.. if score tower_of_fateDestroyedTimer temp matches 1.. run function ssbrc:stages/tower_of_fate/logic/destroy_tower/tick

# Prevent Upper Level Camping
execute if score tower_of_fateDestroyed temp matches 2 run particle minecraft:portal 6000.5 21.5 500.5 5.0 0.0 5.0 0.0 100 normal @a
execute if score tower_of_fateDestroyed temp matches 2 as @a[predicate=ssbrc:flag/targets,predicate=ssbrc:above_y/22] at @s run function ssbrc:stages/tower_of_fate/logic/mystic_barrier/tick
