...
execute if score @s charge.output matches 1 if entity @s[nbt=!{Inventory:[{tag:{hylian_shield:1}}]}] run tag @s add shield_broken
execute if score @s charge.output matches 1 if entity @s[nbt={Inventory:[{tag:{hylian_shield:1},Slot:-106b}]}] run function ssbrc:fighters/link/logic/abilities/sword_spin/check_shield_durability
