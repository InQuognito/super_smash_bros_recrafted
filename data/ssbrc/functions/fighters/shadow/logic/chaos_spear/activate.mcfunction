...
execute if score @s[nbt={Inventory:[{tag:{chaos_blast:1}}]}] shadow.meter.villain < shadow.chaos_meter.max vars run clear @s minecraft:nether_star{chaos_blast:1}
execute if score @s[nbt={Inventory:[{tag:{chaos_control:1}}]}] shadow.meter.hero < shadow.chaos_meter.max vars run clear @s minecraft:nether_star{chaos_control:1}
