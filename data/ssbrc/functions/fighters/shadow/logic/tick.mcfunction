...
execute if entity @s[scores={use_ability=1..,cooldown.1=..0},nbt={SelectedItem:{tag:{chaos_control:1}}}] at @s run function ssbrc:fighters/shadow/logic/chaos_control/on
...
# Chaos Control
scoreboard players remove @a[scores={shadow.chaos_control=1..}] shadow.chaos_control 1
execute as @a[scores={shadow.chaos_control=..0}] run function ssbrc:fighters/shadow/logic/chaos_control/off
...
execute unless score @s[scores={flag.sprinting=..59},predicate=ssbrc:flag/sprinting] shadow.chaos_control matches 1.. run scoreboard players add @s flag.sprinting 1
