...
execute if score chaosEnergy temp >= shadow.chaos_spearCost vars run function ssbrc:fighters/shadow/logic/chaos_spear/cooldown
...
execute unless score @s[tag=!silenced,scores={flag.sprinting=..59},predicate=ssbrc:flag/sprinting] shadow.chaos_control matches 1.. run scoreboard players add @s flag.sprinting 1
