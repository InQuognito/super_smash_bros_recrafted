...
execute if entity @s[nbt={SelectedItem:{tag:{gatlingGun:1,charging:1,powerUp:0}}}] unless score @s charge.output matches 1.. run function ssbrc:fighters/yar/logic/abilities/ray_blaster/variants/gatling_gun/reset
execute if entity @s[nbt={Inventory:[{tag:{gatlingGun:1,charging:1,powerUp:0}}]},nbt=!{SelectedItem:{tag:{gatlingGun:1,powerUp:0}}}] run function ssbrc:fighters/yar/logic/abilities/ray_blaster/variants/gatling_gun/reset
