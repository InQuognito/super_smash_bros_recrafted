...
tellraw @s[tag=!christmas_outfit,advancements={ssbrc:fighters/joker/skins/christmas_outfit=true}] {"translate":"ssbrc.fighters.joker.skin.christmas_outfit","color":"red","clickEvent":{"action":"run_command","value":"/trigger menu set 3"}}
tellraw @s[tag=christmas_outfit] [{"translate":"ssbrc.fighters.joker.skin.christmas_outfit","color":"red"},{"text":" ✔","color":"green"}]
