...
execute unless score teams options matches 1 store result score random.output temp run random value 1..15
execute if score teams options matches 1 store result score random.output temp run random value 1..16
...
$execute if entity @s[tag=!multiheal,nbt=!{Inventory:[{tag:{heal:1}}]},nbt=!{Inventory:[{tag:{multiheal:1}}]}] run function ssbrc:fighters/hero/logic/abilities/spells/heal/choose {slot:$(slot)}
