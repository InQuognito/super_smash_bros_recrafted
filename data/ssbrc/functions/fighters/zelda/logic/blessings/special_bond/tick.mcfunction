...
execute if entity @s[nbt={Inventory:[{tag:{torch_of_wisdom:1}}]}] as @a[tag=!self,predicate=ssbrc:flag/player,predicate=ssbrc:team_match] at @s run function ssbrc:fighters/zelda/logic/shared_passives/torch_of_wisdom
