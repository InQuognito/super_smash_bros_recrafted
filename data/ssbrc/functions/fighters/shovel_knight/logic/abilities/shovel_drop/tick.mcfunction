...
execute unless score @s charge.3 matches 1.. at @s[scores={motionY=..-100}] if block ~ ~-0.1 ~ #ssbrc:passthrough_charge positioned ~-0.4 ~-0.1 ~-0.4 as @e[tag=!self,predicate=ssbrc:flag/targets,dy=0] positioned ~-0.2 ~-0.8 ~-0.2 if entity @s[dy=0] run function ssbrc:fighters/shovel_knight/logic/abilities/shovel_drop/hit
