...
execute positioned ~-0.2 ~-0.2 ~-0.2 as @e[tag=!self,predicate=ssbrc:flag/targets,dx=0] positioned ~-0.6 ~-0.7 ~-0.6 if entity @s[dx=0] run function ssbrc:fighters/toon_link/logic/abilities/boomerang/hit

execute positioned ~-0.2 ~-0.2 ~-0.2 as @a[tag=self,limit=1,dx=0] positioned ~-0.6 ~-0.6 ~-0.6 if entity @s[dx=0] run function ssbrc:fighters/toon_link/logic/abilities/boomerang/regain

scoreboard players add @s temp 1
execute unless score @s temp matches 20.. run teleport @s ^ ^ ^0.6
execute if entity @s[scores={temp=20..}] facing entity @a[tag=self,limit=1] eyes run function ssbrc:fighters/toon_link/logic/abilities/boomerang/return
