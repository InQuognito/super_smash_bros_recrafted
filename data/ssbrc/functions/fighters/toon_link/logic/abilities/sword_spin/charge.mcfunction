...
execute if entity @s[scores={charge.output=21..40}] rotated as @e[type=minecraft:marker,tag=toon_link.spinner,limit=1] run particle minecraft:dust 1.0 1.0 1.0 1.0 ^ ^ ^2 0.0 0.0 0.0 0.0 1 force @a[tag=!self]
execute if entity @s[scores={charge.output=41..}] rotated as @e[type=minecraft:marker,tag=toon_link.spinner,limit=1] run particle minecraft:dust 1.0 1.0 0.0 1.0 ^ ^ ^2.5 0.0 0.0 0.0 0.0 1 force @a[tag=!self]

execute if entity @s[scores={charge.output=1},nbt=!{Inventory:[{tag:{hylian_shield:1}}]}] run tag @s add shieldBroken
