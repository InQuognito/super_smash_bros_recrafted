...
execute positioned ~-0.375 ~-0.375 ~-0.375 as @e[predicate=ssbrc:flag/targets,dx=0] positioned ~-0.25 ~-0.25 ~-0.25 if entity @s[dx=0] as @e[type=minecraft:marker,tag=orangeGrenade,sort=nearest,limit=1] at @s run function ssbrc:fighters/donkeykong/logic/abilities/orange_grenade/explode
...
execute if score @s point > 0 integers unless block ~ ~-0.6 ~ #ssbrc:passthrough run function ssbrc:fighters/donkeykong/logic/abilities/orange_grenade/bounce

execute rotated as @s run teleport @s ^ ^ ^0.5

scoreboard players add @s temp 1
execute at @s[scores={temp=40..}] run function ssbrc:fighters/donkeykong/logic/abilities/orange_grenade/explode
