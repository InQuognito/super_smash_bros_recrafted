...
execute if entity @s[scores={charge.2=1..},predicate=ssbrc:flag/not_on_ground,predicate=ssbrc:flag/sneaking] run function ssbrc:fighters/rob/logic/abilities/robo_burner/tick
effect clear @s[scores={charge.3=1..},predicate=!ssbrc:flag/sneaking] minecraft:levitation

execute if entity @s[scores={charge.3=1..},predicate=!ssbrc:flag/not_on_ground] run function ssbrc:fighters/rob/logic/abilities/robo_burner/deactivate
