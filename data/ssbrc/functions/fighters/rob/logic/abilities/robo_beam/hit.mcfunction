execute if entity @e[type=minecraft:marker,tag=robo_beam,tag=small,sort=nearest,limit=1] store success score damage_success temp run damage @s 8.0 ssbrc:beam by @a[tag=self,limit=1]
