...
execute if score @s charge.1 >= #mario.superJumpThreshold vars if score super_jump_falloff temp >= #mario.superJumpMovementFalloff vars run function ssbrc:fighters/mario/logic/abilities/super_jump/reset

# Stomp
execute store result score @s motionY run data get entity @s Motion[1] 1000

execute unless score @s charge.2 matches 1.. at @s[scores={motionY=..-100}] if predicate ssbrc:flag/in_air positioned ~-0.4 ~-0.1 ~-0.4 as @e[tag=!self,predicate=ssbrc:flag/targets,dy=0] positioned ~-0.2 ~-0.8 ~-0.2 if entity @s[dy=0] run function ssbrc:fighters/mario/logic/abilities/stomp/hit
