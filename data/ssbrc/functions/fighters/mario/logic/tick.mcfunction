execute if entity @s[scores={use_ability=1..,cooldown.1=..0},nbt={SelectedItem:{tag:{fireball:1}}}] at @s anchored eyes run function ssbrc:fighters/mario/logic/abilities/fireball/activate
...
execute unless score @s charge.2 matches 1.. at @s[scores={motion_y=..-100}] if predicate ssbrc:flag/in_air positioned ~-0.4 ~-0.1 ~-0.4 as @e[tag=!self,predicate=ssbrc:flag/targets,dy=0] positioned ~-0.2 ~-0.8 ~-0.2 if entity @s[dy=0] run function ssbrc:fighters/mario/logic/abilities/stomp/hit
