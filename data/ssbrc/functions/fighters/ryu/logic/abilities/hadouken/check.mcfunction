execute as @e[type=#ssbrc:generics,tag=hadouken] if score @s id = @a[tag=self,limit=1] id run tag @s add checkAgainst
execute store result score hadouken temp if entity @e[type=#ssbrc:generics,tag=checkAgainst]
tag @e[type=#ssbrc:generics,tag=hadouken,tag=checkAgainst] remove checkAgainst
