...
execute if entity @s[scores={charge.output=51..},predicate=!ssbrc:fighters/ganondorf/master_sword_awakened] rotated as @e[type=minecraft:marker,tag=ganondorf.spinner,limit=1] run particle minecraft:dust 1.0 0.5 0.0 1.0 ^ ^ ^2 0.0 0.0 0.0 0.0 1 force @a[tag=!self]

execute if score @s charge.output matches 1 if entity @s[nbt=!{Inventory:[{tag:{hylianShield:1}}]}] run tag @s add shieldBroken
execute if score @s charge.output matches 1 if entity @s[nbt={Inventory:[{tag:{hylianShield:1},Slot:-106b}]}] run function ssbrc:fighters/ganondorf/logic/abilities/sword_spin/check_shield_durability
