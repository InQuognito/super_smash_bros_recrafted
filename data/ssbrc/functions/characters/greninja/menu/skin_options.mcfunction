...
tellraw @s[advancements={ssbrc:characters/greninja/shiny=true}] [{"text":"[ ","color":"dark_green"},{"text":"Shiny","color":"green","clickEvent":{"action":"run_command","value":"/execute as @s[tag=!greninja.shiny] run function ssbrc:characters/greninja/menu/costumes/shiny"}},{"text":" ]","color":"dark_green"}]
