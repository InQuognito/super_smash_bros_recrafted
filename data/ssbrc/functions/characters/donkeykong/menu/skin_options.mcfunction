...
tellraw @s[advancements={ssbrc:characters/donkeykong/costumes/gold=true}] [{"text":"[ ","color":"dark_green"},{"text":"Gold","color":"gold","clickEvent":{"action":"run_command","value":"/execute as @s[tag=!donkeykong.default] run function ssbrc:characters/donkeykong/menu/costumes/gold"}},{"text":" ]","color":"dark_green"}]
