...
tellraw @s[advancements={ssbrc:characters/mario/costumes/gold=true}] [{"text":"[ ","color":"dark_green"},{"text":"Gold","color":"gold","clickEvent":{"action":"run_command","value":"/execute as @s[tag=!mario.default] run function ssbrc:characters/mario/menu/costumes/gold"}},{"text":" ]","color":"dark_green"}]
