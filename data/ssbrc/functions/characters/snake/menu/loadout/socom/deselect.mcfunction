...
scoreboard players operation @s[tag=selectingLoadout,tag=snake.socom] snake.capacity -= #snake.socomWeight vars
title @s[tag=selectingLoadout,tag=snake.socom] actionbar [{"text":"You have de-selected the SOCOM.","color":"red"}]

execute if entity @s[tag=snake.socom] run function ssbrc:characters/snake/menu/loadout/display_capacity
