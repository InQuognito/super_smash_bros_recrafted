...
scoreboard players operation @s[tag=selectingLoadout,tag=snake.famas] snake.capacity -= #snake.famasWeight vars
title @s[tag=selectingLoadout,tag=snake.famas] actionbar [{"text":"You have de-selected the Famas.","color":"red"}]

execute if entity @s[tag=snake.famas] run function ssbrc:characters/snake/menu/loadout/display_capacity
