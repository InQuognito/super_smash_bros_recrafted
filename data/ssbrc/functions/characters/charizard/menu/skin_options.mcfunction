...
tellraw @s[advancements={ssbrc:characters/charizard/shiny=true}] [{"text":"[ ","color":"dark_green"},{"text":"Shiny","color":"green","clickEvent":{"action":"run_command","value":"/execute as @s[tag=!charizard.shiny] run function ssbrc:characters/charizard/menu/costumes/shiny"}},{"text":" ]","color":"dark_green"}]
