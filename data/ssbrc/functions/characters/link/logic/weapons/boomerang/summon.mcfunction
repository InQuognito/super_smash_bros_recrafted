...
execute at @s anchored eyes run summon minecraft:spectral_arrow ^ ^-0.3 ^1 {damage:0.4,Tags:["link.boomerang","temp.spawn","projectile"],NoGravity:1b}
...
execute store result entity @e[tag=projectile,limit=1] Motion[0] double 0.0 run scoreboard players get #targetX pos
execute store result entity @e[tag=projectile,limit=1] Motion[1] double 0.0 run scoreboard players get #targetY pos
execute store result entity @e[tag=projectile,limit=1] Motion[2] double 0.0 run scoreboard players get #targetZ pos
