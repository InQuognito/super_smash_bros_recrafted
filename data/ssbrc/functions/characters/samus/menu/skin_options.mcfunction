...
tellraw @s[advancements={ssbrc:characters/samus/costumes/gold=true}] [{"text":"[ ","color":"dark_green"},{"text":"Gold","color":"gold","clickEvent":{"action":"run_command","value":"/execute as @s[tag=!samus.default] run function ssbrc:characters/samus/menu/costumes/gold"}},{"text":" ]","color":"dark_green"}]
