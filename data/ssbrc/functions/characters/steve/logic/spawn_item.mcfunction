...
execute as @e[nbt={Item:{tag:{steve.item.tier:1}}}] at @a[tag=steve.spawner] run spreadplayers ~ ~ 0 25 false @s

execute as @e[nbt={Item:{tag:{steve.item:1}}}] run data merge entity @s {Glowing:1}
execute as @e[nbt={Item:{tag:{steve.item.tier:1}}}] run data merge entity @s {Glowing:1}
