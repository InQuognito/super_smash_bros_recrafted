...
execute as @e[type=!#hooked_entity_lib:ignore, tag=!hooked_entity_lib.id.tagged, distance=..42] at @s run function hooked_entity_lib:id/set

# schedule again, since bobber still exists
schedule function hooked_entity_lib:id/loop 2
