...
execute if data storage ssbrc:temp game.stage{name:"tower_of_fate"} if data storage ssbrc:data option{hazards: true} unless score tower_of_fate.destroyed temp matches 1.. if score game_time.percent temp matches ..50 run function ssbrc:stage/tower_of_fate/logic/lower_tower/start
