...
function math:percentage {output: "#percent temp", val: "#current_stocks temp", div: "#total_stocks temp"}
...
execute if data storage ssbrc:temp game.stage{name: "tower_of_fate"} unless data storage ssbrc:data option{point_limit: -1} if data storage ssbrc:data option{hazards: "true"} unless score #tower_of_fate.destroyed temp matches 1.. if score #percent temp matches ..50 run function ssbrc:stage/tower_of_fate/lower_tower/start
