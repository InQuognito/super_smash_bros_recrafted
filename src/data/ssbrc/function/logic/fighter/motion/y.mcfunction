...
$data modify storage ssbrc:temp cache.motion.safe set value "$(safe_launch)"
attribute @s minecraft:safe_fall_distance modifier remove ssbrc:launched
execute if data storage ssbrc:temp cache.motion{safe:"true"} run return run tag @s add safe_launch
