...
execute if items entity @s[scores={jump=2}] armor.body *[minecraft:custom_data~{temp: {fighter: {fighter: "giegue"}}}] rotated ~ -90 run return run function ssbrc:fighter/giegue/5th_dimensional_slip/raycast/start
...
execute if score @s jump matches 2.. run function ssbrc:logic/fighter/jump/impulse with entity @s equipment.body.components."minecraft:custom_data".temp.fighter
