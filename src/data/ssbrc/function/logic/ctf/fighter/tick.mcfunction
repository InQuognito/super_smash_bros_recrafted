...
execute as @e[type=minecraft:block_display,tag=flag,predicate=ssbrc:owner,distance=..10] if data entity @s data{state: "held"} at @s positioned ~ ~-2 ~ if entity @e[type=minecraft:marker,tag=flag_anchor,predicate=ssbrc:team_match,distance=..1] run function ssbrc:logic/ctf/flag/capture
