tag @s add self
execute if items entity @s container.* minecraft:written_book[minecraft:custom_data~{item: "options"}] in ssbrc:fighter_select as @r[tag=!self,tag=!cannot_host,distance=0..] run function ssbrc:logic/game/options/set
tag @s remove self
