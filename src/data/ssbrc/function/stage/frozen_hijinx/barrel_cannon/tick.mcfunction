execute if entity @s[tag=!contains_player] if entity @a[predicate=ssbrc:player,tag=!barrel_cannon.in,tag=!barrel_cannon.launched,distance=..1.5] run function ssbrc:stage/frozen_hijinx/barrel_cannon/load
