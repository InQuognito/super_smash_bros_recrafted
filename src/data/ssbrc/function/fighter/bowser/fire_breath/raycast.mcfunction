...
execute positioned ~-.5 ~-.5 ~-.5 as @e[tag=!self,predicate=ssbrc:target,dx=0] run function ssbrc:logic/damage/player/fire {amount: 2, burning: 10, type: "pierce", kb_resist: .9, source: "@a[predicate=ssbrc:id_match,limit=1]"}
