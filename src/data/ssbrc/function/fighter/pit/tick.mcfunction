# Guardian Orbitars
execute if items entity @s[tag=!silenced,tag=!guardian_orbitars,scores={cooldown.2=..0},predicate=ssbrc:flag/sneaking] weapon.mainhand minecraft:bow[minecraft:custom_data~{item: "palutena_bow"}] positioned ~ ~.75 ~ rotated ~ 0 run function ssbrc:fighter/pit/guardian_orbitars/activate
...
execute if entity @s[tag=!silenced,scores={timer=1..}] run function ssbrc:fighter/pit/wings/regain/timer
