execute if items entity @s weapon.mainhand *[minecraft:custom_data~{item: "beat_call"}] if entity @s[tag=!silenced,tag=!beat_call,scores={mega_man.beat_call=1..,charge.1=..0}] run return run function ssbrc:fighter/mega_man/beat_call/activate
