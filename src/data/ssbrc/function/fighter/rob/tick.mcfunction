...
execute if score @s charge.1 < rob.robo_beam.max const run function ssbrc:fighter/rob/robo_beam/charge

# Robo Burner
execute if score @s[scores={cooldown.1=..0}] resource < rob.fuel.max const run scoreboard players add @s resource 1

execute if entity @s[tag=!silenced,scores={resource=1..,jump.cooldown=..0},predicate=ssbrc:input/jump] run function ssbrc:fighter/rob/robo_burner/tick
