...
execute positioned ~-.5 ~-.5 ~-.5 as @e[tag=!self,predicate=ssbrc:target,dx=0] run function ssbrc:logic/damage/fire {amount: 12, burning: 60, type: "generic", kb_resist: 0, source: "@a[predicate=ssbrc:owner,limit=1]"}
