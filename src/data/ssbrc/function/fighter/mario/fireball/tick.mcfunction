...
execute positioned ~-.375 ~-.375 ~-.375 as @e[tag=!self,predicate=ssbrc:target,dx=0] positioned ~-.25 ~-.25 ~-.25 if entity @s[dx=0] run function ssbrc:logic/damage/fire {amount: 4, burning: 30, type: "generic", kb_resist: 0, source: "@a[predicate=ssbrc:owner,limit=1]"}
