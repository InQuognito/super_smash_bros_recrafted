...
execute positioned ~-0.3 ~-0.3 ~-0.3 as @e[tag=!self,predicate=ssbrc:target,dx=0] positioned ~-0.4 ~ ~-0.4 if entity @s[dx=0] run function ssbrc:logic/damage/generic {amount: 6, type: "generic", kb_resist: 0, source: " by @a[tag=self,limit=1]"}
...
execute if score @s point matches 1.. unless block ~ ~-.15 ~ #ssbrc:passthrough run function ssbrc:fighter/pikachu/logic/abilities/thunder_jolt/bounce
