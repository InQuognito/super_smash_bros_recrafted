...
execute as @e[tag=!self,predicate=ssbrc:target,distance=..2] unless block ~ ~-.1 ~ #ssbrc:passthrough run function ssbrc:logic/damage/generic {amount: 8, type: "generic", kb_resist: 0, source: "@a[predicate=ssbrc:owner,limit=1]"}
