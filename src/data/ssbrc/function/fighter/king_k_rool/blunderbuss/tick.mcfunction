...
execute if entity @s[tag=!hit_ground] positioned ~-.4 ~-.4 ~-.4 as @e[predicate=!ssbrc:owner,predicate=ssbrc:target,dx=0] positioned ~-.2 ~-.2 ~-.2 if entity @s[dx=0] run function ssbrc:logic/damage/generic {amount: 6, type: "pierce", kb_resist: 0, source: "@a[predicate=ssbrc:owner,limit=1]"}
