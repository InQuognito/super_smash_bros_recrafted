# Chaos Spear
execute if score @s charge.1 matches 1.. anchored eyes positioned ^ ^ ^ run function ssbrc:fighter/shadow/abilities/chaos_spear/charge

# Chaos Blast
execute if score @s charge.2 matches 1.. positioned ~ ~.75 ~ run function ssbrc:fighter/shadow/abilities/chaos_blast/charge
scoreboard players set @s[scores={charge.2=101..}] charge.2 0

# Chaos Snap
execute if score @s[tag=chaos_boost,tag=!silenced,scores={jump.cooldown=..0,cooldown.1=..0},predicate=ssbrc:input/jump] resource >= shadow.chaos_snap.cost const run function ssbrc:fighter/shadow/abilities/chaos_snap/activate
...
scoreboard players set @s[tag=silenced] flag.sprinting 0
