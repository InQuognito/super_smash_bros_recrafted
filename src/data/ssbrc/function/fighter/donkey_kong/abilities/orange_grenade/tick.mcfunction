...
execute positioned ~-.375 ~-.375 ~-.375 as @e[tag=!self,predicate=ssbrc:target,dx=0] positioned ~-.25 ~-.25 ~-.25 if entity @s[dx=0] as @n[type=minecraft:marker,tag=orange_grenade] at @s run function ssbrc:fighter/donkey_kong/abilities/orange_grenade/explode
...
execute if score @s point matches 1.. unless block ~ ~-.6 ~ #ssbrc:passthrough_barrier run function ssbrc:fighter/donkey_kong/abilities/orange_grenade/bounce

execute rotated as @s run teleport @s ^ ^ ^.5

scoreboard players add @s temp 1
execute at @s[scores={temp=40..}] run function ssbrc:fighter/donkey_kong/abilities/orange_grenade/explode
