...
execute store result storage ssbrc:temp cache.rotation[0] float .1 run scoreboard players operation offset temp += random.output temp

execute store result score offset temp run data get storage ssbrc:temp cache.rotation[1] 10
function ssbrc:fighter/pokemon_trainer/logic/squirtle/water_gun/offset
execute store result storage ssbrc:temp cache.rotation[1] float .1 run scoreboard players operation offset temp += random.output temp
