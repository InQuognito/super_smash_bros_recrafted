...
execute positioned ~-.75 ~-.75 ~-.75 as @e[tag=!self,predicate=ssbrc:target,dx=0.5] run function ssbrc:logic/damage/generic {amount: 3, type: "generic", kb_resist: 0, source: "@a[predicate=ssbrc:owner,limit=1]"}
