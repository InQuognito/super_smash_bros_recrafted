execute unless block ^ ^ ^.1 #ssbrc:passthrough run return run function ssbrc:fighter/yar/abilities/missile_launcher/explode

particle minecraft:small_flame ^ ^ ^-.25 0.025 0.025 0.025 0 5 normal @a
particle minecraft:smoke ^ ^ ^-0.3 0 0 0 0 1 normal @a

execute positioned ~-.25 ~-.25 ~-.25 as @n[tag=!self,predicate=ssbrc:target,dx=0] positioned ~-.5 ~-.5 ~-.5 if entity @s[dx=0] at @n[type=minecraft:item_display,tag=missile_launcher,predicate=ssbrc:id_match] run function ssbrc:fighter/yar/abilities/missile_launcher/hit
...
execute if score @s temp matches 100.. run function ssbrc:fighter/yar/abilities/missile_launcher/explode
