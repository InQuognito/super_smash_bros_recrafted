...
execute positioned ~-.5 ~-.5 ~-.5 as @e[tag=!self,predicate=ssbrc:target,dx=0] run function ssbrc:logic/damage/generic {amount: 20, type: "pierce", kb_resist: 0, source: "@a[predicate=ssbrc:id_match,limit=1]"}
