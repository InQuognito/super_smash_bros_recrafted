...
# Drone
execute if items entity @s[scores={cooldown.3=..0}] container.* *[minecraft:custom_data~{item: "drone",command:"activate"}] if score @s yar.drone.health < yar.drone.drone.health.cushion const run function ssbrc:fighter/yar/drone/repair
