...
execute positioned ~-.375 ~-.375 ~-.375 as @e[tag=!self,predicate=ssbrc:target,dx=0] positioned ~-.25 ~-.25 ~-.25 if entity @s[dx=0] run function ssbrc:fighter/luigi/abilities/ice_ball/hit
...
execute if entity @s[scores={point=1..}] unless block ~ ~-0.3 ~ #ssbrc:passthrough run function ssbrc:fighter/luigi/abilities/ice_ball/bounce

execute rotated as @s run teleport @s ^ ^ ^0.3

scoreboard players add @s temp 1
execute if score @s temp matches 120.. run function ssbrc:fighter/luigi/abilities/ice_ball/kill
