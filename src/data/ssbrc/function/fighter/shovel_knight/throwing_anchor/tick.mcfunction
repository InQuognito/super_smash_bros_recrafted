...
execute as @e[tag=!self,tag=!hit_ground,predicate=ssbrc:target,distance=..1] run function ssbrc:logic/damage/generic {amount: 6, type: "pierce", kb_resist: 0, source: "@a[predicate=ssbrc:id_match,limit=1]"}
