...
execute positioned ~-.05 ~-.05 ~-.05 as @e[tag=!self,predicate=ssbrc:target,dx=0] positioned ~-.9 ~-.9 ~-.9 if entity @s[dx=0] run function ssbrc:logic/damage/generic {amount: 3, type: "generic", kb_resist: .8, source: "@a[predicate=ssbrc:owner,limit=1]"}
