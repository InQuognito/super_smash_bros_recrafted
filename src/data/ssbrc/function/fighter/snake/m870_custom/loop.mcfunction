...
execute positioned ~-.05 ~-.05 ~-.05 as @e[tag=!self,predicate=ssbrc:target,dx=0] positioned ~-.9 ~-.9 ~-.9 if entity @s[dx=0] run function ssbrc:logic/damage/generic {amount: 1.25, type: "generic", kb_resist: .25, source: "@a[predicate=ssbrc:id_match,limit=1]"}
