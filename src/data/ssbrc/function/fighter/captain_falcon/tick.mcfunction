...
# Falcon Kick
execute if entity @s[tag=!falcon_kick,scores={silenced=..0,cooldown.2=..0},predicate=ssbrc:flag/sneaking] unless function ssbrc:logic/game/entity/player/if_grounded run function ssbrc:fighter/captain_falcon/falcon_kick/activate
execute if entity @s[tag=falcon_kick] if function ssbrc:logic/game/entity/player/if_grounded run function ssbrc:fighter/captain_falcon/falcon_kick/deactivate
