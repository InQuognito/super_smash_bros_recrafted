...
scoreboard players operation #n temp = #sora.magic const

scoreboard players set magic.queued temp 0
execute if items entity @s weapon.mainhand *[minecraft:custom_data~{item: "keyblade"}] run function ssbrc:fighter/sora/magic/hud/calculate
...
function ssbrc:fighter/sora/magic/hud/check
