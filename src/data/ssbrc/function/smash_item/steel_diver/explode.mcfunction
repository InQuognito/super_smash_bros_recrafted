execute as @e[tag=!self,predicate=ssbrc:target,distance=..1] run function ssbrc:logic/damage/explosion {amount: 6, type: "generic", kb_resist: 0, source: "@a[predicate=ssbrc:owner,limit=1]"}
