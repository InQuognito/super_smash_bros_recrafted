execute as @e[predicate=ssbrc:target,distance=..3] run function ssbrc:logic/damage/explosion {amount: 6, type: "generic", kb_resist: 0, source: "@a[predicate=ssbrc:id_match,limit=1]"}
