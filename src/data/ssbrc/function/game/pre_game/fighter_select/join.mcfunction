execute unless data storage ssbrc:data scene.fighter_select run function ssbrc:game/scene/init {id: "fighter_select"}
...
execute if data storage ssbrc:data option{teams: true} run function ssbrc:game/team/join/red
...
execute unless entity @s[tag=cannot_host] unless items entity @a container.* minecraft:written_book[minecraft:custom_data~{item: "options"}] run function ssbrc:game/options/set
