...
execute positioned ~-.25 ~-.25 ~-.25 as @e[predicate=!ssbrc:owner,predicate=ssbrc:target,dx=0] positioned ~-.5 ~-.5 ~-.5 if entity @s[dx=0] at @n[type=minecraft:item_display,tag=air_shooter,predicate=ssbrc:owner] run function ssbrc:game/damage/generic {amount: 8, kb_resist: 0, i_frames: 0}
