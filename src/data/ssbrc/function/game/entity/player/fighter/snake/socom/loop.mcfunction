...
execute positioned ~-.05 ~-.05 ~-.05 as @e[predicate=!ssbrc:owner,predicate=ssbrc:target,dx=0] positioned ~-.9 ~-.9 ~-.9 if entity @s[dx=0] run function ssbrc:game/logic/damage/generic {amount: 3, kb_resist: .8, i_frames: 0}
execute if score #entity_hit temp matches 1 run return 0

scoreboard players remove #n temp 1
execute if score #n temp matches 1.. positioned ^ ^ ^.25 rotated ~ ~.005 run function ssbrc:game/entity/player/fighter/snake/socom/loop
