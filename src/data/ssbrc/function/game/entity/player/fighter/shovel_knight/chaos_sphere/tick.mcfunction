...
execute positioned ~-.375 ~-.375 ~-.375 as @e[predicate=!ssbrc:owner,predicate=ssbrc:target,dx=0] positioned ~-.25 ~-.25 ~-.25 if entity @s[dx=0] run function ssbrc:game/entity/player/fighter/shovel_knight/chaos_sphere/hit
...
execute if score @s point matches 1.. unless block ~ ~-.15 ~ #ssbrc:passthrough_barrier run function ssbrc:game/entity/player/fighter/shovel_knight/chaos_sphere/bounce
