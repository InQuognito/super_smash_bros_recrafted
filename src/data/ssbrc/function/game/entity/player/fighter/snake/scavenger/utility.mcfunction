$execute unless items entity @s container.* *[minecraft:custom_data~{item: "$(item)"}] run function ssbrc:game/logic/item/give/default {item: "$(item)", class: "default", type: "default"}

$scoreboard players operation @s capacity += #snake.$(item).weight const

function ssbrc:game/entity/player/fighter/snake/load/set

$scoreboard players operation @s weapon_$(id).ammo += #snake.$(item).scavenger const

$tellraw @s ["", {translate: "ssbrc.fighter.snake.scavenger"}," | ", {text: "+", color: "green"}, {score: {name: "#snake.$(item).scavenger", objective: "const"}, color: "green"}," ", {translate: "ssbrc.fighter.snake.scavenger.$(item)", color: "green"}]
