...
execute positioned ~-.375 ~-.375 ~-.375 as @e[predicate=!ssbrc:owner,predicate=ssbrc:target,dx=0] positioned ~-.25 ~-.25 ~-.25 if entity @s[dx=0] run function ssbrc:game/damage/fire {amount: 4, duration: 30, kb_resist: 0, i_frames: 0}
...
execute if predicate ssbrc:flag/grounded run function ssbrc:game/entity/player/fighter/mario/fireball/bounce
