...
# Chaos Spear
execute if score @s charge.1 matches 1.. anchored eyes positioned ^ ^ ^ run function ssbrc:game/entity/player/fighter/shadow/chaos_spear/charge

# Chaos Blast
execute if score @s charge.2 matches 1.. positioned ~ ~.75 ~ run function ssbrc:game/entity/player/fighter/shadow/chaos_blast/charge
scoreboard players set @s[scores={charge.2=101..}] charge.2 0

# Chaos Snap
execute if score @s[tag=chaos_boost,scores={silenced=..0,jump.cooldown=..0,cooldown.1=..0},predicate=ssbrc:input/jump] resource >= #shadow.chaos_snap const run function ssbrc:game/entity/player/fighter/shadow/chaos_snap/activate
...
function ssbrc:game/entity/player/fighter/shadow/speed/tick
