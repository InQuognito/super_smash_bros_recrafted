...
execute positioned ~-.25 ~-.25 ~-.25 as @e[predicate=!ssbrc:owner,predicate=ssbrc:target,dx=0] positioned ~-.5 ~-.5 ~-.5 if entity @s[dx=0] run function ssbrc:game/damage/fire {amount: 4, duration: 40, kb_resist: 0, i_frames: 0}
