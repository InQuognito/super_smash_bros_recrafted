...
execute as @e[type=minecraft:item_display,tag=ssbrc.fgii_graham.root,distance=..1,limit=1] run function ssbrc:game/entity/npc/metroid/fgii_graham/rig/remove/this
kill @s

playsound minecraft:entity.generic.explode ambient @a

schedule function ssbrc:game/entity/npc/metroid/fgii_graham/logic/activate 600t replace
