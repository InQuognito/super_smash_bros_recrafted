...
execute if score @s dialogue matches 1 run function ssbrc:game/logic/npc/dialogue/check with entity @s equipment.body.components."minecraft:custom_data".temp

scoreboard players add @s[scores={popup_timer=1..}] popup_timer 1
execute if score @s popup_timer matches 81.. run function ssbrc:game/logic/ui/popup/clear

execute if dimension ssbrc:smash_plaza positioned ~-.5 ~-.5 ~-.5 run function ssbrc:game/logic/lobby/tick
execute if dimension ssbrc:fighter_select run function ssbrc:game/logic/pre_game/fighter_select/tick
