...
$execute if score @s $(bonus) matches 1.. run data modify storage ssbrc:temp cache.bonuses append value ["", {translate: "ssbrc.game.bonus.$(bonus)", bold: true, color: "gold"}," - ", {score: {name: "#cache", objective: "$(bonus)"}, color: "yellow"}, {text: "₡", color: "yellow"}]
