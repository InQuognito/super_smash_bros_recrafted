...
execute if score #percent temp matches 10 run bossbar set ssbrc:timer color red
...
execute if score #game_time timer matches ..0 run function ssbrc:game/logic/post_game/winner/time

execute if data storage ssbrc:temp game.stage{name: "hollow_bastion"} if data storage ssbrc:data option{hazards: "true"} unless score #hollow_bastion.transformed temp matches 1.. if score #game_time timer matches ..60 in ssbrc:smash/hollow_bastion run function ssbrc:game/stage/hollow_bastion/transform
execute if data storage ssbrc:temp game.stage{name: "tower_of_fate"} if data storage ssbrc:data option{hazards: "true"} unless score #tower_of_fate.destroyed temp matches 1.. if score #percent temp matches ..50 run function ssbrc:game/stage/tower_of_fate/lower_tower/start
