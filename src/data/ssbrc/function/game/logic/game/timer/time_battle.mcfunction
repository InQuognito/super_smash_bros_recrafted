...
execute if data storage ssbrc:temp game.stage{name: "hollow_bastion"} if data storage ssbrc:data option{hazards: "true"} unless score #hollow_bastion.transformed temp matches 1.. if score #game_time timer matches ..60 in ssbrc:smash/hollow_bastion run function ssbrc:stage/hollow_bastion/transform
execute if data storage ssbrc:temp game.stage{name: "tower_of_fate"} if data storage ssbrc:data option{hazards: "true"} unless score #tower_of_fate.destroyed temp matches 1.. if score #percent temp matches ..50 run function ssbrc:stage/tower_of_fate/lower_tower/start
