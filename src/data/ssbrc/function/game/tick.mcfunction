...
function ssbrc:game/lobby/credits/tick

# Tick during certain game stage
execute unless score #game_stage temp matches 2.. as @a run function ssbrc:game/lobby/trigger
execute if score #game_stage temp matches 3 run function ssbrc:game/stage/_logic/tick/before_start with storage ssbrc:temp game.stage
execute if score #game_stage temp matches 4.. run function ssbrc:game/game_tick

# Training Dummy
execute as @e[type=minecraft:text_display,tag=training_dummy.damage_number] at @s run function ssbrc:game/entity/npc/training_dummy/damage_number/tick
...
function ssbrc:game/pre_game/fighter_select/count/tick
