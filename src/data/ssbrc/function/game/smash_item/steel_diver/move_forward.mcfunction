execute unless block ~ ~ ~ #ssbrc:passthrough run return run function ssbrc:game/smash_item/steel_diver/explode
...
execute if score #entity_hit temp matches 1 run return run function ssbrc:game/smash_item/steel_diver/explode

teleport @s[scores={temp=..19}] ^ ^ ^.01
teleport @s[scores={temp=20..}] ^ ^ ^.15

scoreboard players remove #n temp 1
execute if score #n temp matches 1.. at @s run function ssbrc:game/smash_item/steel_diver/move_forward
