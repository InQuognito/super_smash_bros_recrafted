...
# Falcon Kick
execute if entity @s[tag=!falcon_kick,scores={silenced=..0,cooldown.2=..0},predicate=ssbrc:flag/sneaking] unless function ssbrc:game/fighter/_logic/if_grounded run function ssbrc:game/fighter/captain_falcon/falcon_kick/activate
execute if entity @s[tag=falcon_kick] if function ssbrc:game/fighter/_logic/if_grounded run function ssbrc:game/fighter/captain_falcon/falcon_kick/deactivate
