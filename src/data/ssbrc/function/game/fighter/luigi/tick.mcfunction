...
attribute @s[scores={charge.2=1}] minecraft:gravity modifier remove ssbrc:water_run

execute unless predicate ssbrc:flag/in_water rotated ~ 0 if block ^ ^-.2 ^1 minecraft:water run return run attribute @s minecraft:gravity modifier add ssbrc:water_run -1 add_multiplied_total
