...
execute as @e[tag=!self,predicate=ssbrc:target,dx=0] run function ssbrc:game/logic/damage/generic {amount: .75, type: "generic", kb_resist: .95, source: "@a[predicate=ssbrc:owner,limit=1]"}
