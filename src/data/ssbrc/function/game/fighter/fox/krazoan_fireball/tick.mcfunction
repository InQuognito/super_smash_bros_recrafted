...
execute positioned ~-.25 ~-.25 ~-.25 as @e[tag=!self,predicate=ssbrc:target,dx=0] positioned ~-.5 ~-.5 ~-.5 if entity @s[dx=0] run function ssbrc:game/logic/damage/fire {amount: 4, burning: 40, type: "generic", kb_resist: 0, source: "@a[predicate=ssbrc:owner,limit=1]"}
