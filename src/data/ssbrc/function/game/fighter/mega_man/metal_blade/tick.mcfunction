...
execute positioned ~-.5 ~-.5 ~-.5 as @e[tag=!self,predicate=ssbrc:target,dx=0] run function ssbrc:game/logic/damage/generic {amount: 8, type: "pierce", kb_resist: 0, source: "@a[predicate=ssbrc:owner,limit=1]"}
