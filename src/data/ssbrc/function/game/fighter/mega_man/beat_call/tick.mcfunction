...
execute unless entity @s[tag=dead] if score #percent temp matches ..0 run function ssbrc:game/fighter/mega_man/beat_call/commands/entity/dead
...
execute if entity @e[type=minecraft:marker,tag=electric_terrain,distance=..12] run return run function ssbrc:game/fighter/mega_man/beat_call/electric_terrain
function ssbrc:game/fighter/mega_man/beat_call/move with storage ssbrc:temp cache

execute if entity @s[tag=attack,scores={cooldown=..0}] run function ssbrc:game/fighter/mega_man/beat_call/hit
