...
execute as @e[predicate=!ssbrc:owner,predicate=ssbrc:target,distance=..2] at @s unless block ~ ~-.1 ~ #ssbrc:passthrough run function ssbrc:game/logic/damage/generic {amount: 6, kb_resist: 0, i_frames: 0}
