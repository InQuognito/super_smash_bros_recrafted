...
execute positioned ~-.15 ~-.15 ~-.15 as @e[tag=!self,predicate=ssbrc:target,dx=0] positioned ~-.7 ~-.7 ~-.7 if entity @s[dx=0] run function ssbrc:game/logic/damage/fire {amount: 6, burning: 40, type: "generic", kb_resist: 0, source: "@a[predicate=ssbrc:owner,limit=1]"}
